...
function Test-ChocolateyInstallation {
    ...
    try {
        ...
        try {
            ...
            if ($version -ne $latestVersion) {
                Write-Warning "Chocolatey is outdated. Latest version: $latestVersion"
                $issues += "Chocolatey is outdated"
            }
        }
        catch {
            Write-Warning "Could not check for latest Chocolatey version"
        }
    }
    catch {
    ...
    if ($env:ChocolateyInstall) {
        ...
        if (Test-Path $env:ChocolateyInstall) {
            ...
            Write-Error "Chocolatey directory not found at: $env:ChocolateyInstall"
            $issues += "Chocolatey directory missing"
        }
    }
    else {
        Write-Warning "ChocolateyInstall environment variable not set"
...
function Test-SystemRequirements {
    ...
    elseif ($psVersion.Major -ge 5) {
        Write-Warning "Using PowerShell 5.x (consider upgrading to PowerShell 7+)"
        $issues += "PowerShell version could be upgraded"
    }
    else {
        Write-Error "PowerShell version too old (minimum 5.1 required)"
    ...
    if (Test-Administrator) {
        ...
        Write-Warning "Not running as administrator (some operations may be limited)"
    }
    ...
    try {
        ...
        if ($freeSpaceGB -lt 5) {
            Write-Error "Low disk space (less than 5 GB free)"
            $issues += "Low disk space"
        }
        elseif ($freeSpaceGB -lt 10) {
            Write-Warning "Disk space getting low (less than 10 GB free)"
    ...
    try {
        ...
        if ($testConnection) {
            ...
            Write-Warning "Could not reach chocolatey.org"
    ...
    try {
        ...
        if ($packages) {
            ...
            if ($outdated) {
                ...
                Write-Warning "$outdatedCount package(s) have updates available:"
        ...
        Write-Error "Failed to check installed packages: $($_.Exception.Message)"
...
function Test-ChocolateyConfiguration {
    ...
    try {
        ...
        if ($config -match "allowGlobalConfirmation.*true") {
            ...
            Write-Warning "Global confirmation is disabled (may require manual confirmations)"
        }
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Warning "Chocolatey doctor found issues"
            $issues += "Doctor check failed"

            # Show relevant output
            $doctorOutput | Where-Object { $_ -match "ERROR|FAIL" } | ForEach-Object {
                Write-Host "  $_" -ForegroundColor Yellow
            }
        }
    }
    catch {
        Write-Error "Failed to check configuration: $($_.Exception.Message)"
...
function Test-DiskUsage {
    ...
    try {
        if ($env:ChocolateyInstall) {
            ...
            if (Test-Path $chocoPath) {
                ...
                if ($totalSizeMB -gt 5000) {
                    Write-Warning "Chocolatey installation is large (over 5 GB)"
        ...
        Write-Warning "Could not calculate disk usage: $($_.Exception.Message)"
    }
...
function Test-ScheduledTasks {
    ...
    if ($tasks) {
        ...
        Write-Warning "No Chocolatey scheduled tasks configured"
...
function Show-Summary {
    ...
    if ($AllIssues.Count -eq 0) {
        ...
        Write-Host ""
        Write-Warning "╔════════════════════════════════════════════════╗"
        Write-Warning "║  Found $($AllIssues.Count) issue(s) that need attention:        ║"
        Write-Warning "╚════════════════════════════════════════════════╝"
        Write-Host ""
