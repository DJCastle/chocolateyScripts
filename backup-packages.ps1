...
function Backup-Packages {
    ...
    try {
        ...
        if ($packages) {
            ...
            Write-Warning "No packages found to backup"
            return $false
        }
    }
    catch {
        Write-Error "Failed to backup packages: $($_.Exception.Message)"
        return $false
    }
}
...
function Restore-Packages {
    ...
    try {
        ...
        foreach ($pkg in $backup.Packages) {
            ...
            $installed = choco list --local-only $pkg.Name --limit-output
            if ($installed -match $pkg.Name) {
                Write-Warning "$($pkg.Name) is already installed - skipping"
                $skippedCount++
                continue
            }
        ...
        Write-Warning "Skipped (already installed): $skippedCount packages"
        Write-Error "Failed: $failCount packages"
    ...
    if ($backups) {
        ...
        foreach ($backup in $backups | Sort-Object LastWriteTime -Descending) {
            try {
                ...
                Write-Warning "Could not read backup: $($backup.Name)"
            }
        }
    }
    else {
        Write-Warning "No backups found in $DefaultBackupPath"
    }
}
